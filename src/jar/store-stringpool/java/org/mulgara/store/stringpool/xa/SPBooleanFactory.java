...
/**
 * Factory for creating SPBoolean objects.
 *
 * @created 2004-10-04
 *
...
public final class SPBooleanFactory implements SPTypedLiteralFactory {
  ...
  /**
   * Returns the SPBoolean type ID.
   * @return int
   */
  public int getTypeId() {
    return SPBooleanImpl.TYPE_ID;
  }
  ...
  /**
   * Returns a SPBoolean by evaluating the lexicalForm.
   * @param lexicalForm String must be either "true", "false", "1" or "0"
   * @return SPTypedLiteral
   */
  public SPTypedLiteral newSPTypedLiteral(URI typeURI, String lexicalForm) {
    assert SPBooleanImpl.TYPE_URI.equals(typeURI);
    return new SPBooleanImpl(lexicalForm);
  }

  /**
   * Returns a SPBoolean by evaluating the data.
   * @param data ByteBuffer must contain either 1 or 0 as it's first byte
   * @return SPTypedLiteral
   */
  public SPTypedLiteral newSPTypedLiteral(int subtypeId, ByteBuffer data) {
    assert subtypeId == 0;
    return new SPBooleanImpl(data);
  }
