...
public class TranslationTableElem extends D2RQDefn {
  ...
  public TranslationTableElem(Resolver resolver, ResolverSession session, LocalNode ttable, long defModel) throws LocalizeException, QueryException, TuplesException, GlobalizeException {
    ...
    long[] trans = getObjects(ttable.getValue(), session.localize(Constants.translation), defModel);
    for (int i = 0; i < trans.length; i++) {
      String dbValue = getStringObject(new LocalNode(trans[i]),
                                       new LocalNode(session.localize(Constants.dbValue)),
                                       model,
                                       false);
      String rdfValue = getStringObject(new LocalNode(trans[i]),
                                        new LocalNode(session.localize(Constants.rdfValue)),
                                        model,
                                        false);
      db2rdf.put(dbValue, rdfValue);
      rdf2db.put(rdfValue, dbValue);
    }
  }
}
