 ...
 */
public abstract class MulgaraReifier implements Reifier {

  /**
   * The parent graph.
   */
  protected GraphMulgara parent;

  /**
   * The graph that stores the reification triples - maybe the same as the
   * parent graph depending on configuration.
   */
  protected GraphMulgara reificationGraph;
   ...
   */
  public static Reifier create(GraphMulgara parent,
      ReificationStyle style) {

    if (style == ReificationStyle.Convenient) {
      return new MulgaraConvenientReifier(parent, parent.getReificationGraph());
    }
    else if (style == ReificationStyle.Minimal) {
      return new MulgaraMinimalReifier(parent, parent.getReificationGraph());
    }
    else if (style == ReificationStyle.Standard) {
      return new MulgaraStandardReifier(parent, parent.getReificationGraph());
    }
    else {
