 ...
 */
public class MulgaraQuery extends com.hp.hpl.jena.graph.query.Query {

  /**
   * Logger. This is named after the class.
   */
  private final static Logger logger =
      Logger.getLogger(MulgaraQuery.class.getName());
   ...
   */
  public MulgaraQuery(Graph pattern) {
    addMatches(pattern);
  }
  ...
  public ExtendedIterator executeBindings(List outStages, NamedGraphMap args,
    ...
    LocalJenaSession session = ((GraphMulgara) args.get(firstGraph)).getSession();
  ...
  private ModelExpression toModelExpression(Set uniqueGraphs) {
    ...
    if (uniqueGraphs.size() == 1) {
      return new ModelResource(((GraphMulgara) uniqueGraphs.iterator().next()).
          getURI());
    }
    else {
      Iterator graphIter = uniqueGraphs.iterator();
      GraphMulgara tmpGraph;
      ModelExpression exp1, exp2, expression;

      // Create union the first two graphs.
      tmpGraph = (GraphMulgara) graphIter.next();
  ...
  private ConstraintExpression toConstraintExpression(NamedGraphMap args,
    ...
    if (uniqueGraphs.size() == 1) {
      GraphMulgara tmpGraph = (GraphMulgara) uniqueGraphs.iterator().next();
      ...
      while (iter.hasNext()) {
        ...
        GraphMulgara tmpGraph = (GraphMulgara) args.get(graphName);
        Cons tmpCons = cons;
  ...
  /**
   * A HashMap that contains a mapping of a graph to a {@link MulgaraQuery.Cons}
   * objects.
   */
