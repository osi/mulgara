 ...
 */
public class KowariQueryHandler implements QueryHandler {

  /**
   * Logger. This is named after the class.
   */
  private final static Logger logger =
      Logger.getLogger(KowariQueryHandler.class.getName());
   ...
   *   are null.
   */
  public KowariQueryHandler(LocalJenaSession existingDatabaseSession,
      GraphKowari newGraph) throws JenaException {
  ...
  public BindingQueryPlan prepareBindings(Query q, Node[] variables)
    ...
    if (!(q instanceof KowariQuery)) {
      throw new IllegalArgumentException("Can only use KowariQuery objects");
    }

    return new KowariQueryPlan((GraphKowari) graph, (KowariQuery) q, variables, session,
        graph.graphURI);
  }

  public TreeQueryPlan prepareTree(Graph pattern) {
    TreeQueryPlan plan = new KowariTreeQueryPlan(graph, pattern, session,
        graph.getURI());
    return plan;
  }
