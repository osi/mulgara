 ...
 */
public class IteratorHandlerImpl implements IteratorHandler {

  /**
   * The list of unclosed iterators.
   */
  private Set unclosedIterators = new HashSet();
  ...
  public void deregisterIterator(ClosableIterator iterator) {
    if (unclosedIterators.contains(iterator)) {
      unclosedIterators.remove(iterator);
    }
  }
  ...
  public void close() {
    ...
    Iterator iter = unclosedIterators.iterator();
    while (iter.hasNext()) {
      AnswerClosableIteratorImpl tmpIter = (AnswerClosableIteratorImpl) iter.next();
      if (tmpIter != null) {
