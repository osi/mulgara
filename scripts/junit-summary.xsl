...
<xsl:template match="testsuites">
    ...
    <redirect:write file="{$output.dir}/junit-summary.txt">
...
<xsl:template match="testsuites" mode="overview.packages">
 ...
 <xsl:for-each select="testsuite[not(./@package = preceding-sibling::testsuite/@package)]">
   ...
   Tests: <xsl:value-of select="format-number(sum($insamepackage/@tests),'0000')"/> Errors: <xsl:value-of select="sum($insamepackage/@errors)"/> Failures: <xsl:value-of select="sum($insamepackage/@failures)"/> Time: <xsl:call-template name="display-time"> <xsl:with-param name="value" select="sum($insamepackage/@time)"/> </xsl:call-template> - <xsl:value-of select="@package"/> <xsl:if test="@package = ''">&lt;none&gt;</xsl:if> </xsl:for-each>
...
<xsl:template name="display-time">
    ...
    <xsl:value-of select="format-number($value,'00000.000')"/>
</xsl:template>
